...
create table insa(
	...
	sabun 	char(8) not null, 												/* 사번(년(2)월(2)일(2)일련번호(2) */
...
select i.name, i.jikkub, s.bongong from salary s, insa i where s.jikkub = i.jikkub;
select s.bonbong as '본봉' from salary s, insa i where s.jikkub = i.jikkub and i.name = '이기자';
